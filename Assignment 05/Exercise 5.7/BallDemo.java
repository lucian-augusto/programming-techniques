...
public class BallDemo   
{
    ...
    public void boxBounce(int ballAmount) {
        Rectangle box = drawFrame();
        
        ArrayList<BoxBall> balls = new ArrayList<BoxBall>();
        
        for(int i = 0; i < ballAmount; i++) {
            balls.add(new BoxBall(box, myCanvas));
            balls.get(i).draw();
        }
    ...
    private Rectangle drawFrame() {
        if (myCanvas != null) {
            Dimension canvasDimensions = myCanvas.getSize();
            int maxHeight = canvasDimensions.height - 20;
            int maxWidth = canvasDimensions.width - 20;
            
            myCanvas.fill(new Rectangle(0, 0, canvasDimensions.width, canvasDimensions.height));
            
            Rectangle box = new Rectangle(20, 20, maxWidth-20, maxHeight-20);
